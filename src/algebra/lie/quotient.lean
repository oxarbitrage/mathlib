...
noncomputable def quot_ker_equiv_range :
  L ⧸ f.ker ≃ₗ⁅R⁆ f.range :=
{ map_lie' :=
  begin
    rintros ⟨x⟩ ⟨y⟩,
    erw [← set_like.coe_eq_coe, lie_subalgebra.coe_bracket],
    simpa only [linear_map.quot_ker_equiv_range_apply_mk, submodule.quotient.quot_mk_eq_mk,
      coe_to_linear_map, linear_equiv.to_fun_eq_coe, ← f.map_lie],
  end,
